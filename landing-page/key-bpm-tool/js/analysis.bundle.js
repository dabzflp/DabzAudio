(() => {
  ...
  __export(index_esm_exports, {
    RealTimeBpmAnalyzer: () => z,
    analyzeFullBuffer: () => E,
    createRealTimeBpmProcessor: () => Z,
    getBiquadFilter: () => B
  });
  async function c(o, s = 0.2, e = 0.95, n = 0.05) {
    let t = e;
    do
      if (t -= n, await o(t)) break;
    while (t > s);
  }
  ...
  function x({ audioSampleRate: o, data: s, threshold: e, offset: n = 0 }) {
    ...
    for (let r = n; r < a; r += 1) s[r] > e && (t.push(r), r += i);
  ...
  function B(o, s) {
    ...
    return e.type = "lowpass", e.frequency.value = s?.frequencyValue ?? 200, e.Q.value = s?.qualityValue ?? 1, e;
  }
  ...
  function w(o) {
    let s = [];
    for (let e = 0; e < o.length; e++) for (let n = 0; n < 10; n++) {
...
`;
      ...
      z = class {
        ...
        async findPeaks({ audioSampleRate: s, channelData: e, bufferSize: n, currentMinIndex: t, currentMaxIndex: i, postMessage: a }) {
          await c(async (r) => {
            if (this.nextIndexPeaks[r] >= i) return false;
  ...
  async function analyzeAudioUrl(audioUrl, progressCallback = () => {
    ...
    const chromaAvg = chromaSum.map((v2) => v2 / frames);
  ...
  async function estimateBPMWithRealtime(audioBuffer) {
    try {
      const mod = await Promise.resolve().then(() => (init_index_esm(), index_esm_exports));
  ...
  function dot(a, b2) {
    return a.reduce((s, v2, i) => s + v2 * b2[i], 0);
  }
  function normalizeVector(v2) {
