...
1. PARTIES
This agreement is entered into on ${formatDate(contract.effective_date)} between the undersigned parties in connection with the musical work described below.
...
export function registerContractRoutes(app) {
      ...
      for (const s of signers || []) {
        ...
        const { rows: [signer] } = await client.query(
          `INSERT INTO lb_contract_signers
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7)
           RETURNING id, contract_id, name, email, role, master_share_percent, publishing_share_percent, signed_at, created_at`,
          [
            contract.id,
            String(s.name || "").trim(),
  ...
  app.post("/api/contracts/:id/send", requireAuth, async (req, res) => {
    if (!emailEnabled()) {
      return res
        .status(400)
        .json({ error: "Resend is not configured. Set RESEND_API_KEY to send emails." });
    }
    ...
    try {
      ...
      res.json({ sent, total: signerRows.length });
    } catch (err) {
  ...
  app.get("/api/contract-sign", async (req, res) => {
    ...
    try {
      ...
      const { rows: allSigners } = await pool.query(
        `SELECT id, name, email, role, master_share_percent, publishing_share_percent, signed_at
    ...
    try {
      ...
      await client.query(
        `UPDATE lb_contract_signers
            SET signed_at = NOW(), name = COALESCE($1, name)
          WHERE id = $2`,
        [name ? String(name).trim() : null, signer.id]
      );
